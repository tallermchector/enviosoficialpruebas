...
export function RepartidorSelection({ repartidores }: RepartidorSelectionProps) {
    ...
    return (
        <Card className="w-full max-w-md shadow-2xl rounded-none border-border bg-card/50 backdrop-blur-sm">
            <CardHeader className="text-center border-b border-border mb-6 pb-6">
                        ...
                        {repartidores.map(r => (
                            <motion.div key={r.id} variants={itemVariants}>
                                <Button
                                    asChild
                                    variant="outline"
                                    className="w-full justify-between h-16 text-lg rounded-none border-border bg-muted/40 hover:bg-primary hover:text-white hover:border-primary transition-all group font-sans"
                                >
                                    <Link href={`/repartidor/${r.id}`}>
                                        <div className="flex items-center gap-4">
                                            <div className="bg-muted p-2 border border-border group-hover:bg-primary/90 transition-colors">
