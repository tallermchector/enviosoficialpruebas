...
export function NavDropdown({ group }: NavDropdownProps) {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent className="mt-2 w-64 bg-[#0a0d16]/95 backdrop-blur-2xl border-slate-200 text-slate-900 shadow-[0_20px_50px_-15px_rgba(0,0,0,0.8)] p-2">
            ...
            <DropdownMenuItem key={item.href} asChild className="focus:bg-primary/20 focus:text-blue-400 cursor-pointer rounded-lg p-3 transition-colors">
              <Link
                ...
                className={cn(
                  ...
                  isActive(item.href) ? "text-primary font-bold" : "text-slate-600",
                )}
              >
                <div className={cn(
                  "w-8 h-8 rounded-lg flex items-center justify-center transition-colors",
                  isActive(item.href) ? "bg-primary/20 text-primary" : "bg-slate-50 text-slate-500"
                )}>
